...
use C:/Users/Shourish/Documents/PRISM/cropArea_1, clear;
...
merge 1:1 gridNumber using C:/Users/Shourish/Documents/PRISM/linkGridnumber_t_ID;
drop _merge;
compress;
sort gridNumber;
    ...
    forvalues t = `yearMin'/`yearMax' {;
        ...
        * load data;
    	use if (dateNum >= mdy(`monthBegin',`dayMonthBegin',year(dateNum)))
    			...
    			using C:/Users/Shourish/Documents/PRISM/year`t'/state`s'_year`t', clear;
        ...
        * keep only were weights are nonzero;
        ...
        if (_N > 0) {;
            ...
            * collapse by aggregation level;
            collapse tMin tMax tAvg prec dday* [aweight=cropArea], by(t_ID dateNum);

            * collapse by year;
            gen year = year(dateNum);
            collapse (sum) prec dday* (mean) tMin tMax tAvg, by(t_ID year);
            ...
            * save data;
            if (`appendID' > 0) {;
	            append using test1617_ddayByYearandt_ID_croparea1_allstates;
	        };
	        local appendID = 1;
            sort t_ID year;
            qui save test1617_ddayByYearandt_ID_croparea1_allstates, replace;
        };
    };
};
